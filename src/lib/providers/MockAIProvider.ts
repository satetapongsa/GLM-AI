...
export class MockAIProvider implements AIProvider {
  ...
  private generateIntelligentResponse(
    ...
    const responses = [
      {
        reasoning: "กำลังค้นหาข้อมูลในคลังสมองของ GML...",
        text: `กูไม่รู้ (สมชื่อ GML / กูไม่รู้เอไอ เลยครับ) 
...
ตอนนี้ยังไม่ได้เชื่อมต่อ API ของจริงเข้ามา แต่ระบบ UI, การพิมพ์, การสลับโมเดล และโครงสร้างทั้งหมดทำงานพร้อมใช้งาน 100% แล้วครับ`,
      },
      {
        ...
        text: `กูไม่รู้ ขอตอบอย่างตรงไปตรงมาตามคอนเซปต์แบรนด์เลยครับว่าตอนนี้ยังไม่มี API จริงมาป้อนสมอง แต่หน้าเว็บกับระบบแชทตัวนี้พัฒนาขึ้นมาแบบ Production Ready แล้วครับ`,
      },
      {
        reasoning: "Chain of Thought: กำลังประมวลผลข้อความ...",
        text: `ถามกู กูก็ไม่รู้เหมือนกัน 

แต่ถ้าต่อ API ของจริงเมื่อไหร่ ไม่ว่าจะเป็น **Gemini 3.1 Pro**, **Claude Sonnet 5** หรือ **GPT-5.6** ระบบจะตอบได้ฉลาดกว่านี้ 100 เท่าแน่นอนครับ`,
      },
      {
